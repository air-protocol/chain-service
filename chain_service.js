...
const gasCheck = [
  check('fromKey').exists().trim().escape(),
  check('to').exists().trim().escape(),
];

app.post('/gas', gasCheck, async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty())
      return res.status(422).json({ errors: errors.array() });

    const { fromKey, to } = req.body;
    await chain.gas(fromKey, to);
    return res.status(200).send('Send gas finished!');
  } catch (error) {
    return res.status(500).send(error.message);
  }
});

const balanceCheck = [check('address').exists().trim().escape()];

app.get('/balance/:address', balanceCheck, async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty())
    return res.status(422).json({ errors: errors.array() });
